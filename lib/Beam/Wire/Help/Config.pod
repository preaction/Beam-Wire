...
Relative config files will be searched for in the same directories as relative
containers (the C<dir> attribute on the container, or the C<BEAM_PATH> environment variable). If 
the file is not found, you can use C<$default> to fall back to a default.
